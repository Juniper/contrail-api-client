...
class VncApi(object):
    ...
    def resource_list(self, obj_type, parent_id=None, parent_fq_name=None,
        ...
        if fields:
            if detail:
                # when details is true, VNC API return at least all properties
        ...
        for resource_dict in resource_dicts:
            ...
            # if requested ref or backref field not in the result, that means
            # resource does not have ref/backref of that type. Set it to an
            # empty list to prevent VNC client lib to call again VNC API when
            # user uses the get ref/backref method on that types in the
            ...
            for empty_field in empty_fields:
                setattr(resource_obj, empty_field, [])
