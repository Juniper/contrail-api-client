...
def SchemaSyncBuilder(target, source, env):
    ...
    schema_gen_cmd = "python generateds/generateDS.py -f -o %s -g contrail-json-schema %s" % (target_path, str(source[0]))
    ...
    yaml_schema_status_cmd = "git status --porcelain -- %s" % (yaml_schema_path)
    output = subprocess.check_output(yaml_schema_status_cmd, shell=True)
    if output != "":
        dec_str = "#" * 80
