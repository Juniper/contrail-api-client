...
def SetupBuildEnvironment(conf):
    ...
    env.AddMethod(ProtocGenGoFunc, "ProtocGenGo")
...
def goList(env, fmt, module):
    golist_cmd = 'go list -f "%s" -m %s' % fmt, module
    out = GoSetupCommon(env, golist_cmd, True)
    return out.strip()

def ProtocGoBuilder(target, source, env):
    if not env.Detect('protoc'):
        raise SCons.Errors.StopError(
            'protoc Compiler not detected on system')
    protoc = env.WhereIs('protoc')
    protobuf = env.goList('{{ .Dir }}', 'github.com/gogo/protobuf')
    asf = env.goList('{{ .Dir }}', 'github.com/Juniper/asf')
    ...
    gogofaster = "Mgoogle/protobuf/field_mask.proto=%s/types,\
                  Mgoogle/protobuf/wrappers.proto=%s/types,\
                  Mgoogle/protobuf/empty.proto=%s/types,\
                  Mpkg/services/baseservices/base.proto=\
                  %s/pkg/services/baseservices" % (
                          protobuf, protobuf, protobuf, asf)
    for src in source:
        protoc_cmd = protoc + ' plugins=grpc:. ' + str(src) +\
            ' --proto_path=' + protobuf +\
            ' --proto_path=' + protobuf + '/protobuf' +\
            ' --proto_path=' + asf +\
            ' --gogofaster_out=' + gogofaster
        print(protoc_cmd)
        code = subprocess.call(protoc_cmd, shell=True)
        if code != 0:
            raise SCons.Errors.StopError(
...
def ProtocSconsEnvGoFunc(env):
    protoc_gobuild = Builder(action=ProtocGoBuilder)
    env.Append(BUILDERS = {'ProtocGo' : protoc_gobuild})


def ProtocGenGoFunc(env, proto_files):
    ProtocSconsEnvGoFunc(env)
    suffix = '.pb.go'
    targets = []
    for proto_file in proto_files:
        basename = Basename(proto_file)
        targets.append(basename + suffix)
    return env.ProtocGo(targets, proto_files)
