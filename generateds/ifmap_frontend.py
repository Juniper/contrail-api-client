...
class IFMapApiGenerator(object):
    ...
    def _generate_client_classes(self, gen_filepath_pfx, gen_filename_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "        props_dict['fq_name'] = list(self.fq_name) if self.fq_name else None")
            write(gen_file, "        if hasattr(self, 'parent_type'):")
            ...
            for prop in ident.getProperties():
                ...
                xsd_type = prop.getXsdType()
                if complex_type and xsd_type:
                    write(gen_file, "        props_dict['%s'] = self.%s.exportDict('') if self.%s else None" % (prop_name, prop_name, prop_name))
                else:
            ...
            write(gen_file, "               if 'attr' in refs_dict and refs_dict['attr']:")
            write(gen_file, "                   refs_dict['attr'] = refs_dict['attr'].exportDict('')")
