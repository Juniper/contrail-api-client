...
class IFMapApiGenerator(object):
    ...
    def _generate_client_classes(self, gen_filepath_pfx, gen_filename_pfx):
        ...
        for ident in self._non_exclude_idents():
            ...
            write(gen_file, "    def to_dict(self):")
            write(gen_file, "        props_dict = {}")
            write(gen_file, "        # Properties to dictionary")
            write(gen_file, "        props_dict['uuid'] = self.uuid")
            write(gen_file, "        if self.fq_name:")
            write(gen_file, "           props_dict['fq_name'] = list(self.fq_name)")
            write(gen_file, "        else:")
            write(gen_file, "           props_dict['fq_name'] = None")
            write(gen_file, "        if hasattr(self, 'parent_type'):")
            write(gen_file, "           props_dict['parent_type'] = self.parent_type")
            for prop in ident.getProperties():
                prop_name = prop.getName().replace('-', '_')
                complex_type = prop.getCType()
                xsd_type = prop.getXsdType()
                if complex_type and xsd_type:
                    write(gen_file, "        if self.%s is None:" % prop_name)
                    write(gen_file, "           props_dict['%s'] = None" % prop_name)
                    write(gen_file, "        else:")
                    write(gen_file, "           props_dict['%s'] = self.%s.to_dict(None)" % (prop_name, prop_name))
                else:
                    write(gen_file, "        props_dict['%s'] = self.%s" % (prop_name, prop_name))
                write(gen_file, "")
            write(gen_file, "        # References to dictionary")
            write(gen_file, "        for ref in self.ref_fields:")
            write(gen_file, "           if hasattr(self, ref) and getattr(self, ref):")
            write(gen_file, "               refs = getattr(self, ref)")
            write(gen_file, "               props_dict[ref] = []")
            write(gen_file, "               for r in refs:")
            write(gen_file, "                   to = r.get('to')")
            write(gen_file, "                   uuid = r.get('uuid')")
            write(gen_file, "                   attr = r.get('attr')")
            write(gen_file, "                   if attr:")
            write(gen_file, "                       attr = attr.to_dict(None)")
            write(gen_file, "                   props_dict[ref].append({'to': to, 'uuid': uuid, 'attr': attr})")
            write(gen_file, "")
            write(gen_file, "        return props_dict")
            write(gen_file, "    # end to_dict")
            write(gen_file, "")
